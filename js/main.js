...
// ==========================================
// 7. Dynamic Unlocking for Herbs Card based on Horangi Evolvement
// ==========================================
window.addEventListener('load', () => {
    const herbsCard = document.getElementById('card-herbs');
    const isButterflyUnlocked = localStorage.getItem('horangi_butterfly_unlocked') === 'true';
    
    if (herbsCard && isButterflyUnlocked) {
        // Remove locked status, switch to active
        herbsCard.classList.remove('locked');
        herbsCard.classList.add('active');
        herbsCard.removeAttribute('title');
        herbsCard.setAttribute('title', '새싹 삼총사 관찰일지 보기');
        
        const nameEl = document.getElementById('herbs-card-name');
        if (nameEl) nameEl.textContent = "새싹 삼총사 🌱";
        
        const statusEl = document.getElementById('herbs-card-status');
        if (statusEl) {
            statusEl.textContent = "파릇파릇 새싹 삼총사의 성장 일기";
            statusEl.style.color = "var(--accent-color)";
        }
        
        // Hide overlay lock icon
        const lockIcon = herbsCard.querySelector('.lock-icon-container');
        if (lockIcon) {
            lockIcon.style.display = 'none';
        }
        
        // Remove any previous locked event listeners and bind the active one
        // Clone node is the safest way to purge listeners in pure JS
        const newCard = herbsCard.cloneNode(true);
        herbsCard.parentNode.replaceChild(newCard, herbsCard);
        
        newCard.addEventListener('click', () => {
            synth.playChime();
            alert("✨ 축하합니다! 루시가 정성껏 돌본 호랑이 나비 🦋가 보답으로 마당에 떨어뜨려 준 [새싹 삼총사] 레몬밤, 루꼴라, 딜 관찰일지가 곧 정식 개설될 예정입니다! 🌱✨");
        });
    }
});
